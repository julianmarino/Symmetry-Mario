...
//		System.out.println(dir +" 13 Loaded");
	...
	private static void findClassesInJar() throws IOException {
		try {
			JarInputStream in = new JarInputStream(Zones.class.getProtectionDomain().getCodeSource().getLocation().toURI().toURL().openStream());
...
//		System.out.println("fis:"+file.getPath());
		try {
			...
			ArrayList line =  new ArrayList(Arrays.asList(al.get(0).toString().split("\r\n")));
			//sList = al.toArray();
			...
			for(int i=0;i<test.height;i++) {
				...
				line =  new ArrayList(Arrays.asList(al.get(i).toString().split(",")));
                                int tm;
				for(int j=0;j<line.size();j++) {
				    try
				    {
				      String t=line.get(j).toString().replaceAll("[^0-9]", "");
				    ...
				    {
				      System.out.println("NOT A NUMBER:"+line.get(j).toString());
				    }					  
				}
