...
//    }
    ...
    {
        dis.readFully(MyLevel.TILE_BEHAVIORS);
    }

    public static void saveBehaviors(DataOutputStream dos) throws IOException
    {
        dos.write(MyLevel.TILE_BEHAVIORS);
    }

    public static MyLevel load(DataInputStream dis) throws IOException
    {
        long header = dis.readLong();
        if (header != MyLevel.FILE_HEADER) throw new IOException("Bad level header");
    ...
    {
        dos.writeLong(MyLevel.FILE_HEADER);
        dos.write((byte) 0);
