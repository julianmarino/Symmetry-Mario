...
import dk.itu.mario.level.RandomLevelModified;
...
import dk.itu.mario.res.ResourcesManager;
	...
	public class LevelSceneTest extends LevelScene{
			...
			public void init() {
		        ...
		        if(level==null)
		        	if(isCustom){
		        		int num_ODDS_HILL_STRAIGHT=0;
		    	        int num_ODDS_TUBES=0; 
		    	        int num_ODDS_JUMP=0;
		    			int num_ODDS_CANNONS=0; 
		    			int num_BLOCK_ELEMENT=0; 
		    			int num_COINS=0; 
		    			int num_ENEMY_RED_KOOPA=0; 
		    			int num_ENEMY_GREEN_KOOPA=0; 
		    			int num_ENEMY_GOOMBA=0; 
		    			int num_ENEMY_SPIKY=0; 
		    			int num_ENEMY_FLOWER=0; 
		    			int num_ENEMY_ARMORED_TURTLE=0; 
		    			int num_ENEMY_JUMP_FLOWER=0; 
		    			int num_ENEMY_CANNON_BALL=0;
		    			int num_ENEMY_CHOMP_FLOWER=0;
		    			int num_ODDS_TUBES_FLOWER=0;
		    			int num_ODDS_HILL_STRAIGHT_FLOAT=0;
		    			
		    			int num_WOOD=0;
		    			int num_SMALL_TUBE=0;
		    			int num_BLOCK_BLUE=0;
		    			int num_BLOCK_WOOD=0;
		    			
		    			boolean validateGenerationManyScreens=false;
		    			Hashtable hsObjectsScreen;
		        		...
		        		//********mthod for generate beauty screens top k*******//
		        		
		    			if(typeTask==1)
		    			{
		    			currentLevel = new BeautyCustomizedLevel(84, 15, levelSeed, levelDifficulty,levelType,num_ODDS_HILL_STRAIGHT,num_ODDS_TUBES, num_ODDS_JUMP,
		    			...
		    			{
		    			int counterIts=0;
		    			for(num_ODDS_HILL_STRAIGHT=1;num_ODDS_HILL_STRAIGHT<3;num_ODDS_HILL_STRAIGHT++)
		    			{
		    				for(num_ODDS_TUBES=1;num_ODDS_TUBES<3;num_ODDS_TUBES++)
		    				{
		    					for(num_ODDS_HILL_STRAIGHT_FLOAT=1;num_ODDS_HILL_STRAIGHT_FLOAT<2;num_ODDS_HILL_STRAIGHT_FLOAT++)
		    					{
		    						for(num_ENEMY_RED_KOOPA=1;num_ENEMY_RED_KOOPA<3;num_ENEMY_RED_KOOPA++)
		    						{
		        		currentLevel = new BeautyCustomizedLevel(84, 15, levelSeed, levelDifficulty,levelType,counterIts,num_ODDS_HILL_STRAIGHT,num_ODDS_TUBES, num_ODDS_JUMP,
		    	    			num_ODDS_CANNONS, num_BLOCK_ELEMENT, num_COINS, num_ENEMY_RED_KOOPA, num_ENEMY_GREEN_KOOPA, num_ENEMY_GOOMBA, 
		    	    			num_ENEMY_SPIKY, num_ENEMY_FLOWER, num_ENEMY_ARMORED_TURTLE, num_ENEMY_JUMP_FLOWER, num_ENEMY_CANNON_BALL,
		    	    			num_ENEMY_CHOMP_FLOWER,num_ODDS_TUBES_FLOWER,num_ODDS_HILL_STRAIGHT_FLOAT,num_WOOD,num_SMALL_TUBE,num_BLOCK_BLUE,num_BLOCK_WOOD);		        		
		    ...
		    /*
		    			...
		    			int maxScreensEvaluated=16;
		    			ArrayList metricsList=new ArrayList<SingleScreen>();
		    			for(int i=0;i<maxScreensEvaluated;i++)		    				
		    			{
		    				currentLevel = new LoadBeautyLevel(20, 15, levelSeed, levelDifficulty,levelType,"Tela"+i);
			...
			public Hashtable CreateHashTableObjectsScreen(Hashtable hsObjectsScreen,ArrayList objectsOfSpecificType)
			{
				Iterator<SingleElement> nombreIterator = objectsOfSpecificType.iterator();
    	        while(nombreIterator.hasNext()){
    	        	SingleElement elemento = nombreIterator.next();
    	        	String keyhsObjects=elemento.getTypeElement();
    	        	if(hsObjectsScreen.get(keyhsObjects)!=null)
    	        	{
    	        		ArrayList ObjectsOfTypex =(ArrayList<SingleElement>)(hsObjectsScreen.get(keyhsObjects));
    	        		ObjectsOfTypex.add(elemento);
    	        		hsObjectsScreen.put(keyhsObjects, ObjectsOfTypex);
    	        	}
    	        	else
    	        	{
    	        		ArrayList ObjectsOfTypex=new ArrayList<SingleElement>();
    	        		ObjectsOfTypex.add(elemento);
    	        		hsObjectsScreen.put(keyhsObjects, ObjectsOfTypex);
    	        	}
    	        	//System.out.print(elemento.getNameScreen()+"("+elemento.getValueMetric()+" "+elemento.getValueMetricNormalized()+" )  / ");
